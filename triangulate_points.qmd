...
```{python}
basis = pd.read_csv(os.path.join(cfg['base_path'], cfg['axes_output']))
...
for idx1, ax1 in zip(showgoodframes.index, axes.flat):
    ...
    # for j,rsub in plt1.iterrows():
    #   ax1.text(rsub['x'], rsub['y'], rsub['z'], rsub['point'])
