...
```{python}
points_files = pd.read_csv(os.path.join(get_base_path(cfg),
              cfg["points_files_list"]))
```

Double check that the number of cameras from the calibration matches up with the number of cameras in the points file.

In the points file, there should be a column for trial number, columns for each camera, and a column for each output file.
```{python}
assert points_files.shape[1] == len(camgroup.get_names())+2, \
